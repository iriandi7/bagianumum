    ...
    <div class="card">
        <div class="card-body flex flex-col p-6">
            ...
            <div class="card-text h-full ">
                ...
                <form class="space-y-4" method="POST" action="{{ route('admin.peminjaman.update', $peminjaman->id) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="organisasi_id" id="organisasi_id" class="select2 form-control w-full mt-2 py-2">
                            @foreach ($organisasies as $organisasi)
                                <option value="{{ $organisasi->id }}"
                                    {{ $organisasi->id == $peminjaman->organisasi_id ? 'selected' : '' }}
                        ...
                        <select name="ruangan_id" id="ruangan_id" class="select2 form-control w-full mt-2 py-2">
                            @foreach ($ruangans as $ruangan)
                                <option value="{{ $ruangan->id }}"
                                    {{ $ruangan->id == $peminjaman->ruangan_id ? 'selected' : '' }}
                                    class=" inline-block font-Inter font-normal text-sm text-slate-600">
                    ...
                    <div class="input-area relative">
                        ...
                        <select name="peminjaman_status" id="peminjaman_status"
                            class="select2 form-control w-full mt-2 py-2">
                            @php
                                $status = ['Proses', 'Terima', 'Tolak'];
                            @endphp
                            @foreach ($status as $stat)
                                <option value="{{ $stat }}"
                                    {{ $stat == $peminjaman->peminjaman_status ? 'selected' : '' }}
                                    class=" inline-block font-Inter font-normal text-sm text-slate-600">
                                    {{ $stat }}
                                </option>
                            @endforeach
